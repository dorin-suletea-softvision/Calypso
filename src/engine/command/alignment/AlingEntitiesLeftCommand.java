...
public class AlingEntitiesLeftCommand implements CommandInterface {
	private ArrayList<ViewDragableInterface> alignedEntities;
	private ArrayList<Rectangle> boundsBeforeAlign;

	public AlingEntitiesLeftCommand(Set<ViewDragableInterface> selectedEntities) {
	...
	public boolean execute() throws InvalidParameterException {
		...
		// get leftmost item
		int minX = Session.getSelectedSheet().getWidth();
		for (ViewDragableInterface view : alignedEntities) {
			if (view.getBounds().x < minX) {
				minX = view.getBounds().x;
			}
		}
		...
		for (ViewDragableInterface view : alignedEntities) {
			...
			view.setBounds(minX, prevBounds.y, prevBounds.width, prevBounds.height);
		}
