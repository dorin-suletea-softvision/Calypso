...
public class AlingEntitiesTopCommand implements CommandInterface {
	private ArrayList<ViewDragableInterface> alignedEntities;
	private ArrayList<Rectangle> boundsBeforeAlign;

	public AlingEntitiesTopCommand(Set<ViewDragableInterface> selectedEntities) {
	...
	public boolean execute() throws InvalidParameterException {
		...
		// get topmost item
		int minY = Session.getSelectedSheet().getHeight();
		for (ViewDragableInterface view : alignedEntities) {
			if (view.getBounds().y < minY) {
				minY = view.getBounds().y;
			}
		}
		...
		for (ViewDragableInterface view : alignedEntities) {
			...
			view.setBounds(prevBounds.x, minY, prevBounds.width, prevBounds.height);
		}
